...
def sigmlfy(transcription, gloss):
	...
	# For handshapes, set def location to front, palm facing left
...
# Display SyntheSign page with transcription.
def synthesign(request, hns):
	context = { 'hamnosys' : hns, 'sigml' : sigmlfy(hns,"_TEST_") }
	return render(request, 'inventory/synthesign.html', context)
...
def handshape_edit(request, pk):
	...
	if request.method == "POST":
		print(request.POST);
		if request.POST.get("save"): # If user saves edits to handshape
		...
		elif request.POST.get("animate"): # If user wanted to use SyntheSign
			return redirect('synthesign', hns=request.POST.get("hs_transcription"))
...
def handshape_new(request):
	...
	if request.method == "POST":
		print(request.POST)
		if request.POST.get("save"): # If user saves handshape
			new_name = request.POST.get("hs_name")
			new_description = request.POST.get("hs_description")
			new_transcription = request.POST.get("hs_transcription")
